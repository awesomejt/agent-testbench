...
- **cli/** — Go binary (Cobra + Viper). Accepts structured run data from the harness and posts it to the API. Compiles to a single static binary, easy to deploy to test VMs without a runtime dependency. Configuration via `TESTBENCH_*` env vars or `--api-url` flag.
